...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['PHOENIXFORGEAI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['PHOENIXFORGEAI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Phoenixforgeai API.
 */
export class Phoenixforgeai {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['PHOENIXFORGEAI_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('PHOENIXFORGEAI_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PhoenixforgeaiError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Phoenixforgeai client with an apiKey option, like new Phoenixforgeai({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Phoenixforgeai.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PHOENIXFORGEAI_LOG'), "process.env['PHOENIXFORGEAI_LOG']", this) ??
      defaultLogLevel;
  ...
  static Phoenixforgeai = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PhoenixforgeaiError = Errors.PhoenixforgeaiError;
...
Phoenixforgeai.Pets = Pets;
Phoenixforgeai.Store = Store;
Phoenixforgeai.Users = Users;

export declare namespace Phoenixforgeai {
